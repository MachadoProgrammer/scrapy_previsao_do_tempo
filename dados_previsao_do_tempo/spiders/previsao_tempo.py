  ...
  def parse(self, response):
    ...
    for tempo in response.xpath('//span[@class="col day_col"]'):
        if contador < 4:  # Verifica se ainda não atingiu o limite de 3 dias
            yield {
                'dia_atual': tempo.xpath('.//span[@class="subtitle-m"]/text()').get(),
                'temperatura_maxima_e_minima': tempo.xpath('.//span[@class="max changeUnitT"]/text()').get() + " / " + tempo.xpath('.//span[@class="min changeUnitT"]/text()').get(),
                'condicao_atual': tempo.xpath('.//img[@class="simbW"]/@alt').get()
            }
